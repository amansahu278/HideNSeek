...
from steganography.crypto import Crypto
...
class ImageSteg:
    ...
    def __init__(self, filepath, fp = None, destFileName = None):
        self.delimeter = "c3p0"
        self.filepath = filepath
        self.fp = fp
        self.filename = os.path.split(filepath)[1]
        if(destFileName):
            self.dest_filename = destFileName
        else:
    ...
    def loadImageFromPath(self):
        try:
            if self.fp:
                self.image = Image.open(self.fp).convert('RGB')
            else:
